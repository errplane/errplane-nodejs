...
Errplane.prototype.handleClusterUncaughtExceptions = function(die) {
  var self = this;
    ...
    cluster.on("fork", function(worker) {
      worker.on('message', function(message) {
        if(message.exception) {
          self.report("exceptions/" + message.exception.hash, { context: message.exception })
        }
      });
    });
  } else {
    process.on('uncaughtException', function(err) {
      var payload = self._formatException(err);
      process.send({ exception: payload });
...
Errplane.prototype.expressResponseTimeReporter = function() {
  ...
  return function(request, response, next){
    ...
    response.writeHead = function(status, headers) {
      ...
      var path = request.route ? request.route.path : "/undefined"
      path = ((path == "/") ? "/index" : path)

      self.report("controllers/" + path.substr(1).replace(/\//g, "#"), {
